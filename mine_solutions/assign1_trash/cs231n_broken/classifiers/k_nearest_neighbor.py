...
<<<<<<< HEAD
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in xrange(num_test):
      ...
      #######################################################################
      midsums = np.dot(X[i], -2*self.X_train.T)
      full_sums = midsums+np.sum(X[i]**2)+np.sum(self.X_train.T**2,axis=0)
      dists[i] = np.sqrt(full_sums)
      #######################################################################
      #                         END OF YOUR CODE                            #
      #######################################################################
    return dists
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    midsums = np.dot(X, -2*self.X_train.T)
    #print('X.shape: {}'.format(X.shape))
    #print('X_train.shape: {}'.format(self.X_train.shape))
    #print(np.sum(X**2, axis=1).shape)
    #print(np.sum(self.X_train.T, axis=0).shape)
    #print(midsums.shape)
    X_sum = np.sum(X**2,axis=1)
    X_sum = np.reshape(X_sum, (X_sum.shape[0],1))
    broadcasted_X = np.broadcast_to(X_sum, midsums.shape)
    X_train_sum = np.sum(self.X_train.T**2,axis=0)
    X_train_sum = np.reshape(X_train_sum, (X_train_sum.shape[0],1))
    broadcasted_X_train = np.broadcast_to(X_train_sum.T, midsums.shape)
    full_sums = midsums + broadcasted_X + broadcasted_X_train
    dists = np.sqrt(full_sums)
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      closest_X = np.argsort(dists[i], axis=0)[:k]
      closest_y = np.array([self.y_train[l] for l in closest_X])
