...
<<<<<<< HEAD
  def loss(self, X, y=None, reg=0.0):
    ...
    h_1 = relu(np.dot(X, W1) + b1)  # DxH * N*D + (H,)
    h_2 = np.dot(h_1, W2) + b2 # HxC * NxH + (C,)
    max_norm = np.max(np.exp(h_2), axis=1, keepdims=True)
    max_norm = np.reshape(max_norm, (max_norm.shape[0], 1))
    
    scores = h_2 ############# SCORES!
    
    normed_scores = h_2 # - max_norm
    softmax_base = np.sum(np.exp(normed_scores), axis=1)
    softmax_base = np.reshape(softmax_base, (softmax_base.shape[0], 1))
    out = np.exp(normed_scores) / softmax_base
    
    #np.divide(np.exp(normed_scores), softmax_base) #np.exp(h_2)/softmax_base
    ...
    correct_class_score = normed_scores[np.arange(normed_scores.shape[0]), y]
    #loss = np.mean(-1 * np.log( np.exp(correct_class_score) / softmax_base ))
    loss = np.mean(-1 * correct_class_score + np.log( softmax_base ))
    ...
    dout = out
    dout[np.arange(out.shape[0]), y] -= 1
    dout /= out.shape[0]
    
    dW2 = np.dot(h_1.T, dout)
    ...
    dhidden[h_1 <= 0] = 0  # hidden_layer == h_1
    # finally into W,b
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      batch_inx = np.random.choice(np.arange(X.shape[0]), batch_size, replace=True)
      X_batch = X[batch_inx,:]
      y_batch = y[batch_inx]
      #########################################################################
      #                             END OF YOUR CODE                          #
